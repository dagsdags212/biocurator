...
from biocurator.providers import NCBISearchCriteria, UniProtSearchCriteria, SearchCriteria

def preview_command(
    job_name: Annotated[str, typer.Argument(help="Name of the job to preview")],
    config: Annotated[str, typer.Option("--config", "-c", help="Path to the YAML config file")] = "config.yaml",
):
    ...
    console.print(f"[bold cyan]Previewing results for job:[/bold cyan] [bold]{job_name}[/bold]")
    
    for db_name in job.search.databases:
        if db_name not in curator.searchers:
            console.print(f"[yellow]Database '{db_name}' not configured, skipping[/yellow]")
            continue
        ...
        with console.status(f"[bold blue]Searching {db_name.upper()}..."):
            ...
            table = Table(title=f"Results from {db_name.upper()}", show_header=True, header_style="bold magenta")
